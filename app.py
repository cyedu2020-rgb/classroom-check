...
def check_conflicts(schedules):
    ...
    return conflicts, conflicted_indices
...
if not st.session_state.schedules:
    ...
    conflicts, conflicted_indices = check_conflicts(st.session_state.schedules)
    ...
    for idx, item in enumerate(st.session_state.schedules):
        ...
        display_data.append({
            ...
            "is_conflict": idx in conflicted_indices  # 중복 여부 체크용 숨김 열
        })
    
    df = pd.DataFrame(display_data)

    # 중복 행의 '실습 출석일' 셀에 빨간색 배경과 흰색 글씨 적용
    def highlight_conflicts(row):
        styles = [''] * len(row)
        if row['is_conflict']:
            col_idx = row.index.get_loc("실습 출석일 (1주, 9주, 14주)")
            styles[col_idx] = 'background-color: #ff4b4b; color: white; font-weight: bold;'
        return styles

    styled_df = df.style.apply(highlight_conflicts, axis=1)

    # 화면에 스타일 적용된 테이블 출력 (is_conflict 열은 숨김)
    st.dataframe(styled_df, column_config={"is_conflict": None}, use_container_width=True)
